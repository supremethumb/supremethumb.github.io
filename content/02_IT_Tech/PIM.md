...
가. PIM(Processing-In-Memory)의 정의

  

- 폰 노이만 구조의 메모리 벽(Memory Wall)을 극복하기 위해 메모리 내부(DRAM Bank, Cell Array)에 연산기(ALU/MAC)를 직접 집적하여, 데이터 이동 없이 메모리 내부에서 연산을 수행하는 지능형 융합 반도체 기술.
...
- **폰 노이만 병목 해소**: Host(CPU/GPU)와 메모리 간 버스 대역폭 한계(Bandwidth Wall) 및 데이터 전송 지연(Latency) 최소화.
    
      
    
- **에너지 소비 및 발열 절감**: 전체 시스템 에너지의 60% 이상을 차지하는 오프칩(Off-chip) 데이터 이동을 줄여 와트당 연산 효율(TOPS/W) 극대화.
    
      
    
- **Memory-Bound 워크로드 가속**: LLM 추론 디코딩(Token Generation), 추천 시스템(DLRM) 등 대규모 메모리 대역폭을 요구하는 데이터 집약적 AI 연산 최적화.
...
||**SIMD Engine**|벡터 데이터 및 텐서 행렬-벡터 곱셈(GEMV)을 단일 사이클로 처리하는 병렬 유닛|
|**제어/명령**|**PIM Sequencer**|Host 표준 커맨드(JEDEC) 사이에 PIM 연산 명령을 디코딩하여 내부 파이프라인 제어|
||**Instruction Buffer & SFR**|PIM 전용 연산 매크로 명령 및 가중치/상태 레지스터(Special Function Register) 저장|
|**도메인 분류**|**Digital PIM**|Sense-Amp 후단 디지털 신호 기반 연산, 공정 수율 및 신뢰성 우수 (예: HBM-PIM, AiM)|
||**Analog PIM**|Sense-Amp 전단 크로스바(Crossbar) 어레이에서 옴·키르히호프 법칙 기반 초저전력 연산|
...
가. PIM(Processing-In-Memory) vs PNM(Processing-Near-Memory) 비교
...
나. PIM의 한계 극복 및 향후 전망

  

- **공정 통합 한계 극복**: DRAM 커패시터 공정과 고속 Logic 트랜지스터 공정 간의 열화 문제를 3D 패키징 및 하이브리드 본딩(Hybrid Bonding)으로 완화.
    
      
    
- **LLM 서빙(KV Cache) 최적화**: 거대언어모델의 Attention 단계에서 발생하는 대규모 KV Cache 메모리 병목을 전용 PIM 구조로 오프로딩하여 초거대 AI 인프라의 TCO 절감 및 실시간 서빙 가속화.