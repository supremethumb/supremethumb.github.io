...
- **등장배경 및 필요성**:
    ...
    - **양자 우위(Quantum Advantage) 달성**: 쇼어(Shor) 알고리즘(소인수분해), [[쇼어 & 그루버 알고리즘|그로버]](Grover) 알고리즘(비정렬 탐색) 기반 연산 가속
...
|**알고리즘**|**하이브리드 알고리즘 (NISQ)**|VQE(변분 양자 고유값 해결사), QAOA(양자 근사 최적화) 등 QPU와 고전 CPU/[[GPU]]를 상호 보완적으로 연계하여 계산을 최적화|
